...
async def telegram_webhook(
    ...
    text = remove_emojis(message.text)
    if not text:
        # Nothing but emojis/whitespace survived the cleanup.
        logger.info(f"Chat {chat_id} sent an emoji/whitespace-only message")
        await bot.send_message(
            chat_id=chat_id, message_thread_id=message_thread_id, text=INVALID_TEXT_MESSAGE
        )
        return {"ok": True}

    command = Commands.from_value(text.split(maxsplit=1)[0])
    match command:
        case Commands.START:
        ...
        case Commands.INTERVIEW:
            ...
            llm_input = IAInput(
                [
                    ...
                    IAMessage(
                        message="New interview conversation was initiated. What do you need to know about our new client?"
                    ),
                ]
            )
        ...
        case Commands.DOUBTS:
            logger.debug(f"Chat {chat_id} message not implemented")
            await bot.send_message(
                chat_id=message.chat_id,
        ...
        case Commands.PROGRESS:
            ...
            return {"ok": True}  # TODO: route to the progress-tracking flow
        case None:
            interviewer_agent = agent_factory.build_interviewer_agent()
            system_prompt_with_context = interviewer_agent.insert_context("")
            llm_input = IAInput(
