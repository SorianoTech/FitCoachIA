...
- Telegram and LLM collaborators are FastAPI dependencies. `get_bot`, `_create_bot`, `get_settings`, `get_ia_settings`, and `get_llm` are cached so their HTTP connection pools and configuration are shared. Tests override `get_bot`/`get_llm` at the FastAPI app boundary and pass `AsyncMock` collaborators to services.
